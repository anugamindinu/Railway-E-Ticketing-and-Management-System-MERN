...
function Displayticket() {
  ...
  const updateTicket = (id)=>{
    axios.put(`http://localhost:8070/worrentTicket/update/${id}`, updatedData)
    .then(
      ()=>{
  ...
  useEffect(() => {
    axios
      .get("http://localhost:8070/worrentTicket/addticket", {
        params: {
          $limit: 1,
          $sort: {_id: -1},
        },
      ...
      .then((res) => {
        if (res.data.length > 0) {
          ...
          if (classno === 1 && food === "Single Package") {
            calculatedPrice = (numberOfPassengers * 0)+1500;
          }
          else if (classno === 1 && food === "COMBO Package") {
            calculatedPrice = (numberOfPassengers * 0)+1100;
          }
          else if (classno === 1 && food === "FIESTA Package") {
            calculatedPrice = (numberOfPassengers * 0)+1900;
          }
          else if (classno === 1 && food === "Large Burger with App Juice") {
            calculatedPrice = (numberOfPassengers * 0)+500;
          }
          else if (classno === 1 && food === "Biscuits(include free offer)") {
            calculatedPrice = (numberOfPassengers * 0)+1000;
          }
          else if (classno === 1 && food === "Chips Package") {
            calculatedPrice = (numberOfPassengers * 0)+400;
          }
          else if (classno === 1 && food === "None") {
            calculatedPrice = (numberOfPassengers * 0)+0;
          }
  ...
  const handleDelete = () => {
    let id = ticket._id;
    axios
      .delete(`http://localhost:8070/worrentTicket/delete/${id}`)
      .then((res) => {
        setTicket(null);
      ...
      <div className="container">
        <div className="panel pricing-table">
          ...
          <div className="pricing-plan">
            ...
            <ul className="pricing-features">
              ...
              <li className="pricing-features-item">Warrant slip :<img src ={ticket.image}  width="500px" height="500px"/></li>
            </ul>
            ) : (
       ...
       <div>
        ...
        <div>
         ...
         <a href="/allworrent" className="buttonnext" ><span>Get the ticket </span></a> 
       )}
...
{/* update.......      */}
  ...
  <div className="formbold-form-wrapper">
      ...
      <div class="formbold-mb-3">
        ...
        <img width={50} height={50} src={ticket.image} alt="" />
        <input
              type="file"
